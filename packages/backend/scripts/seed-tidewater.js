...
async function main() {
  ...
  // 4. Data Assets
  const assetDefs = [
    { name: 'Customer Account Data', description: 'Customer accounts, addresses, contact info, service agreements', systemId: createdSystems['Oracle CIS'].id, governanceTier: 'GOLD', dataClassification: 'CONFIDENTIAL', retentionPolicy: '7 years after account closure', refreshFrequency: 'DAILY', orgId: companyId },
    { name: 'Billing Records', description: 'Monthly billing cycles, charges, payments, adjustments', systemId: createdSystems['Oracle CIS'].id, governanceTier: 'GOLD', dataClassification: 'CONFIDENTIAL', retentionPolicy: '7 years per SOX', refreshFrequency: 'DAILY', orgId: companyId },
    { name: 'Meter Reading Data', description: 'AMI interval reads, demand data, tamper alerts', systemId: createdSystems['Itron MDM'].id, governanceTier: 'SILVER', dataClassification: 'INTERNAL', retentionPolicy: '5 years', refreshFrequency: 'REAL_TIME', orgId: companyId },
    { name: 'Asset Registry', description: 'Poles, transformers, pipes, valves, pumps — all utility infrastructure', systemId: createdSystems['Maximo'].id, governanceTier: 'SILVER', dataClassification: 'INTERNAL', retentionPolicy: 'Life of asset + 5 years', refreshFrequency: 'DAILY', orgId: companyId },
    { name: 'GIS Network Model', description: 'Geospatial network topology — lines, substations, pressure zones', systemId: createdSystems['ESRI ArcGIS'].id, governanceTier: 'SILVER', dataClassification: 'INTERNAL', retentionPolicy: 'Indefinite', refreshFrequency: 'WEEKLY', orgId: companyId },
    { name: 'SCADA Telemetry', description: 'Real-time sensor data — voltage, flow, pressure, temperature', systemId: createdSystems['OSIsoft PI'].id, governanceTier: 'BRONZE', dataClassification: 'INTERNAL', retentionPolicy: '3 years detailed, 10 years aggregated', refreshFrequency: 'REAL_TIME', orgId: divisions['Tidewater Electric'].id },
    { name: 'Work Order History', description: 'Maintenance work orders, inspections, repairs, crew assignments', systemId: createdSystems['Maximo'].id, governanceTier: 'SILVER', dataClassification: 'INTERNAL', retentionPolicy: '10 years', refreshFrequency: 'DAILY', orgId: companyId },
    { name: 'Financial Transactions', description: 'GL postings, AP/AR, cost centers, budgets', systemId: createdSystems['SAP ERP'].id, governanceTier: 'GOLD', dataClassification: 'RESTRICTED', retentionPolicy: '7 years per SOX', refreshFrequency: 'DAILY', orgId: divisions['Shared Services'].id },
    { name: 'Employee Records', description: 'Employee demographics, positions, compensation, certifications', systemId: createdSystems['HRIS Workday'].id, governanceTier: 'GOLD', dataClassification: 'RESTRICTED', retentionPolicy: '7 years after termination', refreshFrequency: 'DAILY', orgId: divisions['Shared Services'].id },
    { name: 'Water Quality Results', description: 'Lab test results — turbidity, chlorine, bacteria, pH, lead/copper', systemId: null, governanceTier: 'GOLD', dataClassification: 'PUBLIC', retentionPolicy: '10 years per EPA regulation', refreshFrequency: 'DAILY', orgId: divisions['Tidewater Water'].id },
    { name: 'Outage Management Data', description: 'Outage events, affected customers, crew dispatch, restoration times', systemId: null, governanceTier: 'SILVER', dataClassification: 'INTERNAL', retentionPolicy: '5 years', refreshFrequency: 'REAL_TIME', orgId: divisions['Tidewater Electric'].id },
    { name: 'Regulatory Filings', description: 'Rate cases, compliance reports, environmental permits', systemId: createdSystems['SharePoint'].id, governanceTier: 'GOLD', dataClassification: 'CONFIDENTIAL', retentionPolicy: 'Indefinite', refreshFrequency: 'MANUAL', orgId: divisions['Shared Services'].id },
  ];
