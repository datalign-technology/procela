...
describe('audit.service — hash chain', () => {
  ...
  describe('verifyChain()', () => {
    ...
    it('detects content tampering — flipping action on one entry', () => {
      auditService.log(ORG, 'user-1', 'Person', 'p1', 'CREATE');
      auditService.log(ORG, 'user-1', 'Person', 'p1', 'UPDATE');
      auditService.log(ORG, 'user-1', 'Person', 'p1', 'DELETE');
    ...
    it('detects insertion of an unchained entry', () => {
      auditService.log(ORG, 'user-1', 'Person', 'p1', 'CREATE');
      auditService.log(ORG, 'user-1', 'Person', 'p1', 'UPDATE');
    ...
    it('detects an entry missing its entryHash', () => {
      auditService.log(ORG, 'user-1', 'Person', 'p1', 'CREATE');
  ...
  describe('redactPerson()', () => {
    it('replaces userId with [deleted] and re-chains hashes', () => {
      auditService.log(ORG, 'user-1', 'Person', 'p1', 'CREATE');
      ...
      const rehashed = auditService.redactPerson('attacker');
    ...
    it('returns 0 when no entries reference the person', () => {
      auditService.log(ORG, 'u1', 'Person', 'p1', 'CREATE');
      const rehashed = auditService.redactPerson('does-not-exist');
      assert.strictEqual(rehashed, 0);
    });

    it('leaves untouched entries (before the first modified index) bit-identical', () => {
      auditService.log(ORG, 'u1', 'Person', 'p1', 'CREATE');
      ...
      auditService.redactPerson('TARGET');
  ...
  describe('getByEntity()', () => {
    it('returns only entries matching the (entityType, entityId) pair', () => {
      auditService.log(ORG, 'u1', 'Person', 'p1', 'CREATE');
      auditService.log(ORG, 'u1', 'System', 's1', 'CREATE');
      auditService.log(ORG, 'u1', 'Person', 'p2', 'CREATE');
      auditService.log(ORG, 'u1', 'Person', 'p1', 'UPDATE');

      const hits = auditService.getByEntity('Person', 'p1');
