...
describe('demo-seed endpoint', () => {
  ...
  it('seeds the healthcare profile at the same counts as utilities', async () => {
    const res = await request(port, 'POST', '/admin/demo-seed', { industry: 'healthcare' }, 'SUPER_ADMIN');
    assert.strictEqual(res.status, 200);
    assert.strictEqual(res.body.success, true);
    assert.strictEqual(res.body.data.persona.name, 'Dr. Naomi Okafor');
  ...
  it('healthcare plants orphan assets and a failing DQ rule on the same asset as its issue', async () => {
    await request(port, 'POST', '/admin/demo-seed', { industry: 'healthcare' }, 'SUPER_ADMIN');
    const mappedAssetIds = new Set(mappings.filter((m: any) => m.dataAssetId).map((m: any) => m.dataAssetId));
    assert.ok(dataAssets.find((a: any) => a.id === demoId('asset-legacy-registration')));
    assert.ok(dataAssets.find((a: any) => a.id === demoId('asset-claims-csv')));
    assert.strictEqual(mappedAssetIds.has(demoId('asset-legacy-registration')), false, 'Legacy Registration Extract orphaned');
    assert.strictEqual(mappedAssetIds.has(demoId('asset-claims-csv')), false, 'Claims CSV Dump orphaned');

    const failing = dataQualityRules.find((r: any) => r.id?.startsWith(DEMO_ID_SENTINEL) && r.status === 'FAILING');
    const issue = governanceIssues.find((i: any) => i.id?.startsWith(DEMO_ID_SENTINEL));
    assert.ok(failing && issue);
    assert.strictEqual(failing.dataAssetId, demoId('asset-lab-results'));
  ...
  it('seeds the financial profile at the same counts as utilities', async () => {
    const res = await request(port, 'POST', '/admin/demo-seed', { industry: 'financial' }, 'SUPER_ADMIN');
    assert.strictEqual(res.status, 200);
    assert.strictEqual(res.body.success, true);
    assert.strictEqual(res.body.data.persona.name, 'Grace Lin');
  ...
  it('financial plants orphan assets and a failing DQ rule on the same asset as its issue', async () => {
    await request(port, 'POST', '/admin/demo-seed', { industry: 'financial' }, 'SUPER_ADMIN');
    const mappedAssetIds = new Set(mappings.filter((m: any) => m.dataAssetId).map((m: any) => m.dataAssetId));
    assert.ok(dataAssets.find((a: any) => a.id === demoId('asset-legacy-core')));
    assert.ok(dataAssets.find((a: any) => a.id === demoId('asset-fraud-csv')));
    assert.strictEqual(mappedAssetIds.has(demoId('asset-legacy-core')), false, 'Legacy Core Extract orphaned');
    assert.strictEqual(mappedAssetIds.has(demoId('asset-fraud-csv')), false, 'Fraud CSV Dump orphaned');

    const failing = dataQualityRules.find((r: any) => r.id?.startsWith(DEMO_ID_SENTINEL) && r.status === 'FAILING');
    const issue = governanceIssues.find((i: any) => i.id?.startsWith(DEMO_ID_SENTINEL));
    assert.ok(failing && issue);
    assert.strictEqual(failing.dataAssetId, demoId('asset-aml-alerts'));
    assert.strictEqual(issue.dataAssetId, demoId('asset-aml-alerts'), 'issue + failing DQ target one asset');
  });

  it('seeds the government profile at the same counts as utilities', async () => {
    const res = await request(port, 'POST', '/admin/demo-seed', { industry: 'government' }, 'SUPER_ADMIN');
    assert.strictEqual(res.status, 200);
    assert.strictEqual(res.body.success, true);
    assert.strictEqual(res.body.data.persona.name, 'Evelyn Park');
  ...
  it('government plants orphan assets and a failing DQ rule on the same asset as its issue', async () => {
    await request(port, 'POST', '/admin/demo-seed', { industry: 'government' }, 'SUPER_ADMIN');
    const mappedAssetIds = new Set(mappings.filter((m: any) => m.dataAssetId).map((m: any) => m.dataAssetId));
    assert.ok(dataAssets.find((a: any) => a.id === demoId('asset-legacy-tax')));
    assert.ok(dataAssets.find((a: any) => a.id === demoId('asset-incident-csv')));
    assert.strictEqual(mappedAssetIds.has(demoId('asset-legacy-tax')), false, 'Legacy Tax Extract orphaned');
    assert.strictEqual(mappedAssetIds.has(demoId('asset-incident-csv')), false, 'Incident CSV Dump orphaned');

    const failing = dataQualityRules.find((r: any) => r.id?.startsWith(DEMO_ID_SENTINEL) && r.status === 'FAILING');
    const issue = governanceIssues.find((i: any) => i.id?.startsWith(DEMO_ID_SENTINEL));
    assert.ok(failing && issue);
    assert.strictEqual(failing.dataAssetId, demoId('asset-ph-cases'));
    assert.strictEqual(issue.dataAssetId, demoId('asset-ph-cases'), 'issue + failing DQ target one asset');
  });

  it('seeds the logistics profile at the same counts as utilities', async () => {
    const res = await request(port, 'POST', '/admin/demo-seed', { industry: 'logistics' }, 'SUPER_ADMIN');
    assert.strictEqual(res.status, 200);
    assert.strictEqual(res.body.success, true);
    assert.strictEqual(res.body.data.persona.name, 'Omar Reyes');
  ...
  it('logistics plants orphan assets and a failing DQ rule on the same asset as its issue', async () => {
    await request(port, 'POST', '/admin/demo-seed', { industry: 'logistics' }, 'SUPER_ADMIN');
    const mappedAssetIds = new Set(mappings.filter((m: any) => m.dataAssetId).map((m: any) => m.dataAssetId));
    assert.ok(dataAssets.find((a: any) => a.id === demoId('asset-legacy-dispatch')));
    assert.ok(dataAssets.find((a: any) => a.id === demoId('asset-lclaims-csv')));
    assert.strictEqual(mappedAssetIds.has(demoId('asset-legacy-dispatch')), false, 'Legacy Dispatch Extract orphaned');
    assert.strictEqual(mappedAssetIds.has(demoId('asset-lclaims-csv')), false, 'Claims CSV Dump orphaned');

    const failing = dataQualityRules.find((r: any) => r.id?.startsWith(DEMO_ID_SENTINEL) && r.status === 'FAILING');
    const issue = governanceIssues.find((i: any) => i.id?.startsWith(DEMO_ID_SENTINEL));
    assert.ok(failing && issue);
    assert.strictEqual(failing.dataAssetId, demoId('asset-customs'));
    assert.strictEqual(issue.dataAssetId, demoId('asset-customs'), 'issue + failing DQ target one asset');
  });

  it('seeds the insurance profile at the same counts as utilities', async () => {
    const res = await request(port, 'POST', '/admin/demo-seed', { industry: 'insurance' }, 'SUPER_ADMIN');
    assert.strictEqual(res.status, 200);
    assert.strictEqual(res.body.success, true);
    assert.strictEqual(res.body.data.persona.name, 'Priya Anand');
  ...
  it('insurance plants orphan assets and a failing DQ rule on the same asset as its issue', async () => {
    await request(port, 'POST', '/admin/demo-seed', { industry: 'insurance' }, 'SUPER_ADMIN');
    const mappedAssetIds = new Set(mappings.filter((m: any) => m.dataAssetId).map((m: any) => m.dataAssetId));
    assert.ok(dataAssets.find((a: any) => a.id === demoId('asset-legacy-policy')));
    assert.ok(dataAssets.find((a: any) => a.id === demoId('asset-siu-csv')));
    assert.strictEqual(mappedAssetIds.has(demoId('asset-legacy-policy')), false, 'Legacy Policy Extract orphaned');
    assert.strictEqual(mappedAssetIds.has(demoId('asset-siu-csv')), false, 'SIU CSV Dump orphaned');

    const failing = dataQualityRules.find((r: any) => r.id?.startsWith(DEMO_ID_SENTINEL) && r.status === 'FAILING');
    const issue = governanceIssues.find((i: any) => i.id?.startsWith(DEMO_ID_SENTINEL));
    assert.ok(failing && issue);
    assert.strictEqual(failing.dataAssetId, demoId('asset-claims-loss'));
    assert.strictEqual(issue.dataAssetId, demoId('asset-claims-loss'), 'issue + failing DQ target one asset');
  });

  it('seeds the telecom profile at the same counts as utilities', async () => {
    const res = await request(port, 'POST', '/admin/demo-seed', { industry: 'telecom' }, 'SUPER_ADMIN');
    assert.strictEqual(res.status, 200);
    assert.strictEqual(res.body.success, true);
    assert.strictEqual(res.body.data.persona.name, 'Tomás Vega');
  ...
  it('telecom plants orphan assets and a failing DQ rule on the same asset as its issue', async () => {
    await request(port, 'POST', '/admin/demo-seed', { industry: 'telecom' }, 'SUPER_ADMIN');
    const mappedAssetIds = new Set(mappings.filter((m: any) => m.dataAssetId).map((m: any) => m.dataAssetId));
    assert.ok(dataAssets.find((a: any) => a.id === demoId('asset-legacy-provisioning')));
    assert.ok(dataAssets.find((a: any) => a.id === demoId('asset-churn-csv')));
    assert.strictEqual(mappedAssetIds.has(demoId('asset-legacy-provisioning')), false, 'Legacy Provisioning Extract orphaned');
    assert.strictEqual(mappedAssetIds.has(demoId('asset-churn-csv')), false, 'Churn CSV Dump orphaned');

    const failing = dataQualityRules.find((r: any) => r.id?.startsWith(DEMO_ID_SENTINEL) && r.status === 'FAILING');
    const issue = governanceIssues.find((i: any) => i.id?.startsWith(DEMO_ID_SENTINEL));
    assert.ok(failing && issue);
    assert.strictEqual(failing.dataAssetId, demoId('asset-usage-cdr'));
    assert.strictEqual(issue.dataAssetId, demoId('asset-usage-cdr'), 'issue + failing DQ target one asset');
  });

  it('seeds the education profile at the same counts as utilities', async () => {
    const res = await request(port, 'POST', '/admin/demo-seed', { industry: 'education' }, 'SUPER_ADMIN');
    assert.strictEqual(res.status, 200);
    assert.strictEqual(res.body.success, true);
    assert.strictEqual(res.body.data.persona.name, 'Dr. Helen Voss');
  ...
  it('education plants orphan assets and a failing DQ rule on the same asset as its issue', async () => {
    await request(port, 'POST', '/admin/demo-seed', { industry: 'education' }, 'SUPER_ADMIN');
    const mappedAssetIds = new Set(mappings.filter((m: any) => m.dataAssetId).map((m: any) => m.dataAssetId));
    assert.ok(dataAssets.find((a: any) => a.id === demoId('asset-legacy-registrar')));
    assert.ok(dataAssets.find((a: any) => a.id === demoId('asset-survey-csv')));
    assert.strictEqual(mappedAssetIds.has(demoId('asset-legacy-registrar')), false, 'Legacy Registrar Extract orphaned');
    assert.strictEqual(mappedAssetIds.has(demoId('asset-survey-csv')), false, 'Survey CSV Dump orphaned');

    const failing = dataQualityRules.find((r: any) => r.id?.startsWith(DEMO_ID_SENTINEL) && r.status === 'FAILING');
    const issue = governanceIssues.find((i: any) => i.id?.startsWith(DEMO_ID_SENTINEL));
    assert.ok(failing && issue);
    assert.strictEqual(failing.dataAssetId, demoId('asset-controlled-research'));
    assert.strictEqual(issue.dataAssetId, demoId('asset-controlled-research'), 'issue + failing DQ target one asset');
  });

  // ── Governance depth — seeded for every industry at parity ──

  for (const industry of ['utilities', 'shipbuilding', 'healthcare', 'manufacturing', 'financial', 'government', 'logistics', 'insurance', 'telecom', 'education'] as const) {
