...
| `scorecard-value-drivers.png` | Governance → Council Scorecard | The **Governance value drivers** card (ROI Phase 1) — the four leading-indicator tiles (Ownership coverage, Value at risk, Resolved 30 days, Avg days to resolve) with the LEADING INDICATORS pill. Element-clip of the card, not the whole page. Tidewater Utilities fixture. |
| `scorecard-value.png` | Governance → Council Scorecard | The **Estimated governance value** card (ROI Phases 2–3) — the four monetized tiles stamped YOUR ASSUMPTIONS above the By value stream attribution table. Requires a value model set on Governance → Foundation → Value model. Element-clip of the card. Tidewater Utilities fixture. |
