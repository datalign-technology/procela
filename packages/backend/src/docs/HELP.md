...
- Executive Report PDF download. The Executive Report tab has a Download branded PDF action that hits a server-side renderer (catalog counts, coverage signals, governance tier mix, top orphan assets, recent audit activity). The output is a fixed, brand-stamped artefact you can attach to an email or audit ticket — beats the browser's print-to-PDF for compliance use. A secondary Print button still falls back to the browser print dialog if you prefer that style.
