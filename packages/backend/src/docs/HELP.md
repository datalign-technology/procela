...
- Diagram / Cards toggle — opens on the Diagram view by default. The diagram lays nodes out as horizontal swimlanes with edges between lanes so you can see how the org is wired together at a glance. Cards is the dense, searchable alternative (a summary-tile grid with a search box).
