...
- My Dashboard — Your personal view. **Tasks**, **Issues**, and **My Domains** are each their *own* customizable section now (reorder, set **Half**/**Full** width, or hide from Customize), rather than tiles baked into one block, alongside **Needs My Attention** and **My Schedule** below. Each links through — Tasks → Governance Work (Tasks tab), Issues → the Issues tab, My Domains → the Data Domains page, Upcoming Events → the Governance Calendar. Zero-count sections still link through to the destination's empty state.
...
- All / Governed lens — when a governance program scope is defined (Governance → Foundation), the header toggle narrows the you-scoped Portfolio Health, Coverage, and Domains sections to the domains and assets the program governs, with a "Governed scope · v{N} · changed {date}" note. With no scope defined it's a safe no-op showing everything you own or steward.
...
- **The meeting-prep landing page at Insights → Review → Council Dashboard.** Where the Council *Scorecard* is the detailed, point-in-time division grid with monthly snapshots, the **Council Dashboard** is the one screen a governance council opens *before* a meeting — a live briefing composed from the surfaces that own each part, with no data of its own.
- **The council** — the roster: who sits on the council and in what seat (Chair, Vice-chair, Secretary, Member, Advisor), pulled from the governance group. Drills into Governance → Groups.
- **Next meeting** — the next scheduled council meeting: date, how many days away, cadence, and the expected attendees, from the Calendar.
- **Governance health now** — the current coverage / classification / status per division (the live Scorecard derive), so you walk in knowing where the program stands without opening the full grid.
- **Maturity trend** — an inline sparkline of the overall maturity score over time plus the latest per-dimension scores, so you can see whether things are trending up.
- **For the council** — what needs a decision this period (the auto-derived escalations), each drilling back to where it's fixed.
