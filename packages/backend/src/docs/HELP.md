...
- Bipartite visualization of every activity ↔ data asset link in scope. Activities on the left grouped by parent process; mapped data assets on the right grouped by system. Each mapping renders as a coloured curve between the two columns — green for produces, blue for consumes, purple for transforms, grey for references. Read at a glance: "which assets does this step touch", and inversely "which steps use this asset". Reachable from the sidebar (Processes → Process ↔ Data Map).
