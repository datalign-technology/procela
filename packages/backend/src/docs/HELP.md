...
- BCM + measurable-target attributes (advanced view, activity level). Four extra fields sit next to Automation / Est. Duration when the tree is in advanced mode: **Criticality** (Tier 1 mission-critical, Tier 2 business-critical, Tier 3 standard, Tier 4 non-critical, or Not rated), **RTO (hours)** for the paired Recovery Time Objective, **Success Measure** (free-text — "resolve outage tickets within 4h P95"), and **SLA Target** (free-text — "P95 4h", "99.9% monthly", etc.). Together they answer BCM regulator questions like "what's the RTO for outage triage?" that Procela couldn't otherwise produce, and give dashboards a measurable target next to the narrative business outcome.
...
- Tabbed: My Reports (the report catalog + Builder, default tab), Executive Report (printable one-page overview), and Scorecard (governance health by dimension). The old Analysis tab was dropped — cube pivots live on their own page at Explore → Analysis; framing them as a sub-section of Reports was a dead-end nav.
- Executive Report printing. The Executive Report tab has a Print (browser) button that opens the browser print dialog. The report is print-optimised: the application chrome (sidebar, top bar, the Reports header and tabs, the skip-to-content link, the keyboard-shortcut hint) is stripped, and the five sections are compacted so the whole overview lands on a single page — ready to save as PDF or hand to a reviewer.
