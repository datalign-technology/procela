    ...
    <div style={{ maxWidth: 820, margin: '0 auto' }}>
      ...
      <div style={sectionStyle}>
        ...
        <ul style={listStyle}>
          ...
          <li><strong>Assignments tab.</strong> DAMA governance role assignments (CDO, Data Governance Lead, Data Owner, Stewards, etc.). Sidebar filter by role type with live counts; search box matches person, organization, or role label. <strong>Group by</strong> toggle (always visible, even with zero assignments, styled like the app's other segmented controls): By Person (roles as chips), By Role, or Flat (legacy bulk-select) — each shows a genuinely different layout. <strong>By Role is a catalog view</strong> — it lists every governance role even when no one holds it yet, each with a <em>Unfilled</em> marker and an inline <em>+ Assign</em>, so this page tells the same story as the Governance Groups expected-role slate (the roles always exist; assignment is what varies). A brand-new org with groups created but nobody assigned sees the full role list here, not an empty page.</li>
