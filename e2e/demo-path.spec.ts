...
test.describe('Procela demo path', () => {
  test('Beat 1: a populated org renders the analytical dashboard', async ({ page }) => {
    ...
    await expect(page.locator('body')).toContainText(/My Portfolio Health/i, { timeout: 5_000 });
    await expect(page.locator('body')).toContainText(/My Coverage/i, { timeout: 5_000 });
